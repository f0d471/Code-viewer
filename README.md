...
面向视觉-空间型学习者（VSL）的代码阅读 skill。用可视化的方式建立对代码库的完整心智模型。
...
```
scan（全局概览）→ deep（模块解剖）→ flow（数据流追踪）→ ask（即时答疑）
```

从全局地图出发，逐步深入，最后用零散提问查漏补缺。
...
| 步骤 | 命令 | 你获得什么 |
|------|------|-----------|
| 1 | `scan` | 全局地图，知道项目分几层、每个子系统干什么 |
| 2 | `deep` | 核心模块的逐行解读，能从零重写 |
| 3 | `flow` | 一个"有意义的故事"，理解数据怎么流 |
| 4 | `ask` | 零散问题的即时答疑，查漏补缺 |
