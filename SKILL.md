...
VSL 学习者推荐"自顶向下 + 目标驱动"的探索路径：

1. **ask**：先问一个具体问题（"这个函数为什么这么写？""用户登录后发生了什么？"），建立初步理解
2. **flow**：追踪一个具体操作的完整流程，在"有意义的故事"中理解代码
3. **scan**：看全貌，建立全局心智模型
4. **deep**：深入感兴趣的模块，逐行解读核心逻辑

探索漏斗：`ask`（即时答疑）→ `flow`（数据流追踪）→ `scan`（全局概览）→ `deep`（模块解剖）
...
- 当用户直接说 `/code-viewer` 无其他关键词，默认走 ask 流程（推荐 VSL 探索顺序：先问问题再看全貌）。
