...
- 三个报告各有推荐分批方案（含 deep 子系统模式分批方案）
- 每次暂停明确告诉用户下一步
- 每个 section 前重新 Read 依赖文件

### 7. Shared CSS 提取 ✅

`templates/shared-styles.css` 作为 CSS 唯一来源，三个模板只保留各自独有的样式。避免三份模板维护同一套 CSS。

### 8. Deep 子系统模式 ✅

deep 命令新增子系统模式。自动判断：
- 路径下直接包含源代码文件 → 模块模式（M0-M8）
- 路径下包含多个子目录且每个有源代码 → 子系统模式（S1-S5）
- 用户可 `--subsystem` 强制子系统模式

子系统模式 5 步：模块清单 → 调用关系 → 数据流 → 模块边界 → 输出报告

### 9. CodeMap 论文特征落地 ✅

从 arXiv 2504.04553v3 提取三个可借鉴特征：
1. **多粒度导航**：scan（全局）→ deep 子系统（中间层）→ deep 模块（单文件）→ ask（单问题）
2. **目标驱动选择**：scan 报告 §6「你的目标是什么？」4 卡片引导用户选命令
3. **Objective 策略 ~60% 覆盖率**：deep M6 热度筛选，核心块太多时只覆盖最重要的 ~60%

---

## code-viewer 独有（已实现）

- 四种命令的具体流程（scan/deep/flow/ask）✅
- 洋葱模型分层逻辑 ✅（core/onion-model.md）
- deep 双模式（模块 + 子系统）✅
- flow 场景类型检测 ✅
- 入口点速查（entry-points.md）— 已创建，待按项目填充
- 各语言的入口识别规则 — 在各命令文件中内联
